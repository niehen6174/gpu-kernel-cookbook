...
```bash
cd gpu-kernel-lab/operators/layernorm/cuda && bash build.sh
# 或指定架构
CUDA_ARCH=sm_90 bash build.sh
```

### 10.2 运行正确性测试 + Benchmark

```bash
# 从项目根目录运行
cd gpu-kernel-lab
python -m operators.layernorm.test
```

---

## 11. Benchmark 结果（H20，B=4096，N=1024，float32）

H20 理论峰值带宽：~4.0 TB/s

| 实现 | 延迟 | 带宽 | vs PyTorch |
|------|------|------|------------|
| PyTorch | 0.0316 ms | 2123 GB/s | 1.00x |
| Triton | 0.0329 ms | 2037 GB/s | 0.96x |
| CUDA v1 (two-pass) | 0.0922 ms | 728 GB/s | 0.34x |
| CUDA v2 (Welford) | 0.0845 ms | 794 GB/s | 0.37x |

注：
- PyTorch/Triton 接近，均在 2000+ GB/s，利用率约 50% 带宽
- CUDA v1/v2 手写实现约为 PyTorch 的 1/3，说明 vendor 库有更多底层优化（如 vectorized load、persistent kernel 等）
- Welford v2 比 v1 略快，原因是从 2 趟全局内存读改为 1 趟，减少了约 25% 的读取量
